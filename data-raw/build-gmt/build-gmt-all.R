...
# remove redundant db
gmt.files2 <- gmt.files2[!gmt.files2%in% c("data-raw/extdata/gmt//MGI_Mammalian_Phenotype_2017_AUTOGENERATED.txt",
"data-raw/extdata/gmt//MGI_Mammalian_Phenotype_Level_4_AUTOGENERATED.txt")]
...
split_string <- strsplit(names(gmt.all), "[_.-]")

# Remove "from" and "AUTOGENERATED" from split strings
split_string <- lapply(split_string, function(x) gsub("from|autogenerated|of|automatic|RNASEQ|[0-9]{4}", "", x, ignore.case = TRUE))

split_string <- sapply(split_string, function(x) paste(x, collapse = "_"))

# substitute two or more __ for one
...
# remove _UP and down with gsub
names(gmt.all) <- gsub("_UP", "", names(gmt.all))
names(gmt.all) <- gsub("_DOWN", "", names(gmt.all))
...
suffix <- sub(".*:","",names(gmt.all))
prefix_curated <- gset_dbs_curated$PREFIX_SHORT_NAME[match(prefix, gset_dbs_curated$db)]

suffix <- gsub("HALLMARK_","", suffix)

names(gmt.all) <- paste(prefix_curated, suffix, sep = ":")
